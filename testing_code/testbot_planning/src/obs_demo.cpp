...
//   virtual_r_model->distance_factor_ = 0.3;
  ...
  visual_tools.publishText(text_pose, "Obstacle Demo", rvt::WHITE, rvt::XLARGE);
  ...
  ROS_INFO_NAMED("Obstacle Demo", "Reference frame: %s", move_group.getPlanningFrame().c_str());
  
  // We can also print the name of the end-effector link for this group.
  ROS_INFO_NAMED("Obstacle Demo", "End effector link: %s", move_group.getEndEffectorLink().c_str());
...
//   move_group.setPlannerId("RRT");
  ...
  quaternion = tf::createQuaternionMsgFromRollPitchYaw(0,pi,0);
  ...
  ROS_INFO_NAMED("Obstacle Demo", "Visualizing plan 1 (pose goal) %s", success ? "" : "FAILED");

  // Visualizing plans
  // ^^^^^^^^^^^^^^^^^
  // We can also visualize the plan as a line with markers in RViz.
  ROS_INFO_NAMED("Obstacle Demo", "Visualizing plan 1 as trajectory line");

  visual_tools.publishTrajectoryLine(my_plan.trajectory_, joint_model_group);
  visual_tools.trigger();
  visual_tools.prompt("Press 'next' end the demo");
