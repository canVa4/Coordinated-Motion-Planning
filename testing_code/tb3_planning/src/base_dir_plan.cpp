...
int main(int argc, char** argv)
{
  ...
  // const std::string PLANNING_GROUP = "tb3";
...
//   for (std::size_t i = 0; i < classes.size(); ++i)
//     ss << classes[i] << " ";
//   ROS_INFO_STREAM("Exception while loading planner '" << planner_plugin_name << "': " << std::endl << "Available plugins: " << ss.str());

//   std::vector<std::string> algs;
//   planner_instance->getPlanningAlgorithms(algs);
//   for (int i = 0; i < algs.size(); ++i) {
//     ROS_INFO_STREAM("algs :"<< algs[i] << std::endl);
//   }
  ...
  visual_tools.trigger();
  planning_interface::MotionPlanRequest req; //same as moveit_msgs/MotionPlanRequest
  planning_interface::MotionPlanResponse res;


  geometry_msgs::PoseStamped pose;
  pose.header.frame_id = "virtual_base_link";
  ...
  req.planner_id = "RRTControl";
  req.group_name = PLANNING_GROUP;
  // req.goal_constraints.push_back(pose_goal);

  robot_state::RobotState goal_state(robot_model);
  std::vector<double> joint_values = { 0.3, 0.4, 0};
  goal_state.setJointGroupPositions(joint_model_group, joint_values);
  moveit_msgs::Constraints joint_goal = kinematic_constraints::constructGoalConstraints(goal_state, joint_model_group, 0.1, 0.1);
  ...
  /* Set the state in the planni  robot_state::RobotState goal_state(robot_model);
  std::vector<double> joint_values = { -1.0, 0.7, 0.7, -1.5, -0.7, 2.0, 0.0 };
  goal_state.setJointGroupPositions(joint_model_group, joint_values);
  moveit_msgs::Constraints joint_goal = kinematic_constraints::constructGoalConstraints(goal_state, joint_model_group);
  req.goal_constraints.clear();
  req.goal_constraints.push_back(joint_goal);ng scene to the final state of the last plan */
